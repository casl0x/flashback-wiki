...
export default function Sidebar({ versions, selected, counts, totalChars, totalPlayers, totalRels, onSelect }: Props) {
  ...
  return (
    <aside className={styles.sidebar}>
          ...
          >
            <div className={styles.verBtnL}>
              <div className={styles.verBadge} style={{ background: bg, color: fg }}>
                {v.id === 'all'
                  ? <i className="ti ti-layout-grid" style={{ fontSize: 10 }} aria-hidden="true" />
                  : v.id
                }
              </div>
              <span className={styles.verName}>{v.id === 'all' ? 'Tout voir' : v.label}</span>
            </div>
            <span className={styles.verCount}>{v.id === 'all' ? totalChars : counts[v.id] || 0}</span>
          </div>
        )
      })}
      ...
      <div className={styles.sbDivider}>
        ...
        <div className={styles.sbStatRow}><span className={styles.sbStatLabel}>Personnages</span><span className={styles.sbStatVal}>{totalChars}</span></div>
        <div className={styles.sbStatRow}><span className={styles.sbStatLabel}>Joueurs</span><span className={styles.sbStatVal}>{totalPlayers}</span></div>
